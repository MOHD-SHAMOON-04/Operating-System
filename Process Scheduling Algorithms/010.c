// write a simple program to implement highest response ratio next (preemptive) algorithm and 
...
float calculateResponseRatio(int waitingTime, int remainingTime) {
    return (float)(waitingTime + remainingTime) / remainingTime;
}
    ...
    while (completed < n) {
        ...
        for (int i = 0; i < n; i++) {
            if (p[i].arrivalTime <= currentTime && p[i].remainingTime > 0) {
                int waitingTime = currentTime - p[i].arrivalTime;
                float rr = calculateResponseRatio(waitingTime, p[i].remainingTime);
                if (rr > maxRR) {
                    maxRR = rr;
                    idx = i;
                }
