// write a program to implement round robin scheduling algorithm having variables time quantum and find the avg turnaround time, waiting time, 
...
int main() {
    ...
    while (completed < n) {
        ...
        if (p[idx].remainingTime <= timeQuantum) {
            ...
            for (int i = 0; i < n; i++) {
                if (p[i].arrivalTime > currentTime - timeQuantum && p[i].arrivalTime <= currentTime && visited[i] == 0) {
                    queue[rear++] = i;
                    visited[i] = 1;
                }
            }

            queue[rear++] = idx; // Put the same process again at end
        }
    }
