// write a simple program to implement shortest remaining time first scheduling algorithm (shortest job first preemptive) and find the avg turnaround time, 
...
struct Process {
    ...
    int waitingTime;
    int responseTime;
    int started; // flag to check if process started
};
...
int main() {
    ...
    while (completed < n) {
        int idx = -1;
        int minRT = 100000; // very large value initially
